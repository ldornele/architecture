...
| # | Change | Partner Action | Impact if Missed | Classification | Ticket | Parent |
|---|--------|---------------|-----------------|----------------|--------|--------|
| 13 | **OCM SDK removed; standalone JWT handler** | Remove OCM-specific auth config (`server.acl`, `server.authz`); switch to JWT config (issuer_url, audience, identity_claim) | **Auth setup fails.** Entire `pkg/client/ocm/` and `pkg/config/ocm.go` removed; old config fields have no fallback. Helm values for ACL and authz no longer exist | Doc-only (1163/1179) | [HYPERFLEET-492](https://redhat.atlassian.net/browse/HYPERFLEET-492) | - |
